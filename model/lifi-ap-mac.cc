...

{
 ...
 // Simulator::Schedule (MicroSeconds (100),&LiFiApMac::StartSuperFrame, this);
  ...
  Simulator::Schedule (NanoSeconds (10),&LiFiApMac::StartSuperSessions, this);
}

void LiFiApMac::DoDispose ()
{
  m_channel = 0;
  m_bps = 0;
  m_MacLow = 0;
  //std::vector<Ptr<Node> > markedForHandoverNodesVector;
  if (markedUeNodesMap.size() > 0)
  {
    markedUeNodesMap.clear();
  }
  
  m_beaconJitter = 0; //!< UniformRandomVariable used to randomize the time of the first beacon
...
void LiFiApMac::StartSuperSessions (void)
{
 NS_LOG_FUNCTION (this);
  ...
  m_activeEpoch = true;
  frame_number =0;
  Simulator::Schedule (NanoSeconds (100),&LiFiApMac::TransmitControlFrames, this);
  //Simulator::Schedule (MicroSeconds (100),&LiFiApMac::DistributeSlots, this); SendSlotControlPackets
}

void LiFiApMac::TransmitControlFrames ()
{
  ns3::Simulator::ScheduleNow (&LiFiApMac::SendOneBeacon, this);
  double control_reply_wait = 130.0;
  ns3::Simulator::Schedule (MicroSeconds (control_reply_wait),&LiFiApMac::DistributeSlots, this);
}
 ...
 // TransmitControlFrames(0);
  if (transmissionsVector.size() > 0)
  {
   // std::cout<<"void LiFiApMac::SendControlPackets () transmissionsVector.size() "<<transmissionsVector.size()<<" Time Now "<<ns3::Simulator::Now().GetSeconds()<<std::endl;
    Simulator::Schedule (MicroSeconds (100),&LiFiApMac::TransmitDataFrame,this,transmissionsVector[0]);
  }
}

void LiFiApMac::TransmitSlotInfos(uint32_t index)
{
  NS_LOG_FUNCTION (this);
  ...
  else
  {
   //std::cout<<"void LiFiApMac totalTransmissionTimeUs "<<totalTransmissionTimeUs.GetSeconds()<<" GetControlGuardSlotTime "<<GetControlGuardSlotTime()<<" m_slotPtrs_vector "<<m_slotPtrs_vector.size()<<std::endl;
    Simulator::Schedule (totalTransmissionTimeUs, &LiFiApMac::TransmitSlotInfos, this, index);
    index++;
...
void LiFiApMac::DistributeSlots()
{
 ...
 // std::cout<<"void LiFiApMac::DistributeSlots() m_slotPtrs.size() "<<this->m_slotPtrs.size()<<std::endl;
  ...
  for(std::map<uint32_t, Ptr<LiFiUserDeviceInfos>  >::iterator it = m_slotPtrs.begin(); it != m_slotPtrs.end(); ++it)
  {
    ...
    {
        ac_index = GetNumberOfSlotsForAccessCategory (udInfos->GetQoSRequirements(), "Downlink");
    } 
    else 
    {
      ac_index = 1;
...
//    std::cout<<"ac_index  "<<ac_index<<" leng_arr [0] "<<leng_arr[0]<<" leng_arr [1] "<<leng_arr[1]<<std::endl;
 ...
 // leng_arr[0] = (transmissionsVector.size()-1);
  for(std::map<uint32_t, Ptr<LiFiUserDeviceInfos>  >::iterator it = m_slotPtrs.begin(); it != m_slotPtrs.end(); ++it)
  {
    ...
    {
        ac_index = GetNumberOfSlotsForAccessCategory (udInfos->GetQoSRequirements(), "Uplink");
    } 
    else 
    {
      ac_index = 1;
  ...
  Simulator::Schedule (MicroSeconds (10),&LiFiApMac::SendSlotControlPackets, this);
...
void LiFiApMac::TransmitDataFrame (MacTransmissionUnit userDevice)
{
  ...
  if (m_super_frame_number == frame_number)
  {
    NS_LOG_DEBUG ("Starting over all sessions again");//StartFrameTransmission
    //std::cout<<"frame_number: "<<frame_number<<" GetTotalSlotsAllowed "<<GetTotalSlotsAllowed()<<" totalTransmissionTimeUs + GetInterFrameTimeInterval ())*2+GetGuardTime () "<<((totalTransmissionTimeUs + GetInterFrameTimeInterval ())*2+GetGuardTime ())<<std::endl;
    Simulator::Schedule ((totalTransmissionTimeUs + GetInterFrameTimeInterval ())+GetGuardTime (), &LiFiApMac::StartSuperSessions, this);
    m_current_slot = 0;
    m_current_device_index = 0;
  ...
  //std::cout<<"void LiFiApMac::TransmitDataFrame (MacTransmissionUnit userDevice) total transmission time "<<(totalTransmissionTimeUs + GetInterFrameTimeInterval ())<<std::endl;
    Simulator::Schedule ((totalTransmissionTimeUs + GetInterFrameTimeInterval ())*2, &LiFiApMac::StartFrameTransmission, this);
    m_current_slot = 0;
    m_current_device_index = 0;
...
uint8_t LiFiApMac::GetNumberOfSlotsForAccessCategory (uint8_t ac,std::string str)
{
  ...
  if (str == "Downlink")
  { 
    uint32_t slots_per_user = (max_slots-2*m_slotPtrs_vector.size())/m_slotPtrs_vector.size();
    numOfSlotsAllotted = slots_per_user;
  }
  else if (str == "Uplink")
  {
...
//std::cout<<"void LiFiApMac::Receive (Ptr< Packet> packet,WifiMacHeader *hdr)"<<std::endl;
  //WifiMacHeader hdr;
